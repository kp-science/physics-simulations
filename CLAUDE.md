...
- `Virtual Physics Lab 06/` — **ไฟฟ้ากระแสและวงจร** (topic `current` · เริ่ม Lab 108) · access `vlab:vpl06:lab-N` · ต่อสาย kp-auth + admin + protect + labs_data แล้ว · หน้า catalog `virtual-physics-lab-06.html` generate จาก `SERIES_PAGES` เหมือน VPL05 · มิเตอร์เป็น**มัลติมิเตอร์ KP-DMM วางราบบนโต๊ะ 2 ตัว** (สลับหน้าปัดเข็ม/ดิจิทัลด้วยปุ่ม 🔢) · Lab 110 = แซนด์บ็อกซ์เบรดบอร์ด ผู้เรียนวางอุปกรณ์เอง แก้วงจรด้วยวิธีวิเคราะห์โหนดใน `solveCircuit()`
...
## 🔄 เวอร์ชันของสคริปต์ที่แชร์กัน (cache busting)

ทุกหน้าโหลด `kp-auth.js` แบบมีเลขเวอร์ชันท้าย URL:

```html
<script src="../kp-auth.js?v=20260919"></script>
```

`?v=` **ไม่ใช่ไฟล์ใหม่** — เซิร์ฟเวอร์ยังส่ง `kp-auth.js` ตัวเดิม แต่เบราว์เซอร์มองว่าเป็น URL ใหม่ จึงไม่ใช้สำเนาเก่าที่เก็บไว้
(ถ้าไม่มี `?v=` เครื่องที่เคยเข้าเว็บจะใช้ของเก่าอยู่ ~10 นาทีบน GitHub Pages — เคยทำให้เข้าใจผิดว่าแก้สิทธิ์ไม่สำเร็จมาแล้ว)

**ทุกครั้งที่แก้ `kp-auth.js` ต้องรัน:**

```bash
python3 _admin/bump_auth_version.py
```

- ไม่ใส่อาร์กิวเมนต์ = ใช้วันที่วันนี้ (ถ้าซ้ำของเดิมจะต่อ `-2`, `-3` ให้เอง) · ใส่เลขเองก็ได้
- `--check` = ดูว่าตอนนี้เวอร์ชันอะไร และมีไฟล์ไหนยังไม่มี `?v=`
- เลขปัจจุบันเก็บที่ `_admin/asset_version.txt` · เพิ่มไฟล์ที่ต้องติดเวอร์ชันได้ที่ `TARGETS` ในสคริปต์
- `protect_new_file.py` เติม `?v=` ให้ไฟล์ใหม่อัตโนมัติ และ `--scan --fix` จะเติมให้ไฟล์ที่ยังขาด (issue `AUTH_VER`)
...
### 🔁 สิทธิ์ของสมาชิกที่ล็อกอิน = ค่าที่เก็บไว้ + preset ของ role (เฉพาะขอบเขตที่ยังไม่เคยตัดสินใจ)

`migrateAccess()` ใน `kp-auth.js` เรียก `mergeRolePreset(access, role)` ทุกครั้งที่โหลดข้อมูลผู้ใช้:

- ถ้า `access` ที่เก็บไว้**ไม่มี entry ไหนพูดถึงขอบเขตนั้นเลย** (เช่นไม่มีทั้ง `vlab:vpl04:*` และ `vlab:vpl04:lab-xx`) → ถือว่า admin ยังไม่เคยตัดสินใจ จึงเติมให้ตาม `ROLE_ACCESS_PRESETS[role]`
- ถ้ามี entry ของขอบเขตนั้นอยู่แล้ว (แม้เป็นรายไอเทม) → **เคารพค่าเดิม** ไม่เติมทับ
- `role === 'blocked'` → `[]` เสมอ · `access` ที่มี `'*'` อยู่แล้ว → คงเดิม

**ผลคือ**: เพิ่ม series ใหม่ (เช่น vpl04) แล้วสมาชิกเก่าปลดล็อกให้อัตโนมัติ ไม่ต้องไล่แก้รายคน
**ข้อควรระวัง**: จะ "ไม่ให้สิทธิ์ชุดหนึ่ง" ด้วยการเว้นว่างไม่ได้อีกแล้ว — ต้องติ๊กเฉพาะรายการที่อนุญาตในชุดนั้นอย่างน้อย 1 ข้อ หรือใช้ role `blocked`
**หมายเหตุ**: ปุ่มเปลี่ยน role ในแอดมิน (`setRole`) เขียนแค่ฟิลด์ `role` ไม่แตะ `access` — การแก้ `access` ต้องทำผ่าน modal ⚙️ แก้ไขสิทธิ์ แล้วกดบันทึก
**การ์ด 🌐 สิทธิ์ผู้เข้าชมทั่วไป มีผลกับคนที่ไม่ล็อกอินเท่านั้น** (Firestore `settings/public.anonymous_access`) ไม่เกี่ยวกับสมาชิกที่ล็อกอินอยู่
...
## 🐢 ปุ่มปรับความเร็วการแสดงผล

ทุกแลปมีปุ่ม `🐢 ความเร็ว` (×1 → ×½ → ×¼ → ×⅒) ให้ชะลอภาพเพื่อดูการเคลื่อนที่ให้ทัน — ค่าที่เครื่องมือวัดได้ไม่เปลี่ยน เพราะฟิสิกส์และนาฬิกาในแลปใช้เวลาจำลองชุดเดียวกัน

- **Lab 62–78** มีปุ่มในตัว (`simSpeed` / `setSimSpeed()` / `simDt(dt)` ในไฟล์ · ขณะลากด้วยมือใช้เวลาจริง)
- **แลปอื่นทั้งหมด** ใช้ `_shared/kp-speed.js` — ชะลอ timestamp ของ `requestAnimationFrame` และ `performance.now()` แล้วแทรกปุ่มเองข้างปุ่มมุมมอง (หรือสร้างแถบใต้ canvas)
- `protect_new_file.py` แทรก `kp-speed.js` ให้ไฟล์แลปใหม่อัตโนมัติ (ข้ามไฟล์ที่มี `setSimSpeed` เอง)
